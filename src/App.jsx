...
// NOTE: We'll re-import with Settings below to avoid duplicate imports.
import Papa from "papaparse";

/**
 * Math App – React Prototype (Enhanced with Auto-Advance Delay, Ad Timer & Restart)
 */

// ======= Google Sheet CSV endpoints =======
const SHEETS = {
  classes:
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setError(e?.message || "Грешка при зареждане на данните");
      } finally {
...
// ======= UI building blocks =======
function SquareButton({ icon: Icon, label, onClick }) {
  return (
    <button onClick={onClick} className="group flex flex-col items-center justify-center rounded-2xl border bg-white p-6 shadow-sm hover:shadow-md transition w-full aspect-square">
      ...
      <div className="mx-auto max-w-4xl px-4 py-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <Button variant="outline" size="sm" onClick={onLogout} className="ml-2">
            <LogOut className="h-4 w-4 mr-2" />Смени профил
          </Button>
          <Button variant="ghost" size="icon" onClick={onOpenSettings} title="Настройки" className="group">
            <Settings className="h-6 w-6 text-gray-700 transition-transform group-hover:rotate-90 group-hover:text-blue-500" />
          </Button>
        </div>
      </div>
    </div>
      ...
      <div className="mx-auto max-w-md px-4 py-10">
        ...
        <div className="space-y-4">
          <div>
            ...
            <Input placeholder="напр. Алекс" value={name} onChange={(e) => setName(e.target.value)} />
          </div>
          <div>
            ...
            <Select onValueChange={setClassId} value={classId}>
              ...
              <SelectContent>
                {classes.map((c) => (
                  <SelectItem key={normalizeId(c.id) || c.name} value={normalizeId(c.id) || c.name}>
          ...
          <Button className="w-full" onClick={() => onDone({ name: name.trim(), classId })} disabled={!canContinue}>
            Продължи
          </Button>
        </div>
...
function Quiz({ lesson, questions, onFinish, settings }) {
  ...
  // Clear pending auto-advance on unmount / index change
  useEffect(() => {
    return () => {
  ...
  // Timer (overall quiz)
  useEffect(() => {
    if (!settings?.timeLimitMin) return; // no timer
    if (timeLeft <= 0) {
      submit();
      return;
    }
  ...
  // Start/stop ad countdown when final modal opens/closes
  useEffect(() => {
  ...
  function computeScore() {
    ...
    for (const q of qlist) {
      if ((answers[q.id] || "").toString().trim().toUpperCase() === (q.__correctKey || "").toString().trim().toUpperCase()) {
        correct += 1;
      }
    }
  ...
  if (!current) {
    return <div className="p-6 text-center">Няма въпроси за този урок.</div>;
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-4">
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          <div className="text-sm text-slate-600">
            Урок: <span className="font-medium text-slate-800">{lesson.title || lesson.name}</span>
          </div>
        ...
        <CardContent className="p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {opts.map((o) => (
              <Button
                key={o.key}
                variant={chosen === o.key ? (isCorrect ? "default" : "destructive") : "outline"}
                className="h-auto py-3 justify-start"
                onClick={() => choose(o.key)}
              >
                <span className="font-semibold w-6">{o.key}.</span>
                <span className="ml-2 text-left">{o.text}</span>
              </Button>
            ))}
          </div>
          {showExplain ? (
      ...
      <div className="flex items-center justify-between">
        {index < total - 1 ? (
          <Button onClick={() => setIndex(index + 1)}>Следващ въпрос</Button>
        ) : (
          <Button onClick={() => setShowConfirm(true)}>Приключи</Button>
        )}
      ...
      <Dialog open={showConfirm} onOpenChange={setShowConfirm}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Готов ли си да предадеш теста?</DialogTitle>
            <DialogDescription>Провери обобщението по-долу и натисни „Предай“.</DialogDescription>
          </DialogHeader>
          ...
          <div className="mt-4 mb-4 text-sm text-slate-700">
            Отговорени: <b>{Object.keys(answers).length}</b> · Пропуснати: <b>{total - Object.keys(answers).length}</b> · Общо: <b>{total}</b>
          </div>
          <DialogFooter>
            <Button onClick={submit}>Предай</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
...
function TheoryScreen() {
  return (
    <div className="max-w-3xl mx-auto p-6 text-slate-700">
      <h2 className="text-xl font-semibold mb-3">Теория</h2>
      <p>Тук може да показваме резюмета по уроци или да вградим външни материали. В момента е placeholder.</p>
    </div>
  );
}
...
function ResultsScreen({ results, classes, lessons, canRestart, onRestart }) {
  const byLessonId = useMemo(() => {
    const map = new Map(lessons.map((l) => [normalizeId(l.id), l]));
    return (id) => map.get(normalizeId(id));
  }, [lessons]);

  if (!results?.length) {
    return <div className="p-6 text-slate-600">Все още няма предадени тестове.</div>;
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="space-y-3">
        {results.map((r, i) => {
          const l = byLessonId(r.lesson?.id);
      ...
      <div className="mt-6">
        <Button onClick={onRestart} disabled={!canRestart}>Започни отначало</Button>
      </div>
    </div>
  );
}
...
function StatsScreen({ results }) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <StatCard label="Опити" value={totals.attempts} />
        <StatCard label="Вярни" value={totals.solved} />
        <StatCard label="Въпроси" value={totals.totalQs} />
        <StatCard label="Точност" value={`${totals.accuracy}%`} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <Tabs defaultValue="by-lesson" className="w-full">
        ...
        <TabsContent value="by-lesson" className="pt-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {currentLessons.map((l) => {
              const qs = listQuestionsForLesson(l.id);
              return (
                <Card key={normalizeId(l.id)} className="hover:shadow-md transition">
                  <CardContent className="p-4 flex items-center justify-between">
                    <div>
                      <div className="font-medium">{l.title || l.name}</div>
                      <div className="text-xs text-slate-500">Въпроси: {qs.length}</div>
                    </div>
                    <Button size="sm" onClick={() => onStartQuiz(l, qs)} disabled={!qs.length}>
        ...
        <TabsContent value="by-class" className="pt-4">
          <div className="space-y-4">
            {classList.map((cls) => (
              <div key={cls.id}>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Персонализирай тестовете според предпочитанията си.</DialogDescription>
        </DialogHeader>
        <div className="space-y-4 py-2">
          <div className="flex items-center justify-between">
            <span className="text-sm">Показвай обяснение след отговор</span>
            <input type="checkbox" checked={!!settings.showExplanation} onChange={(e)=>setSettings({...settings, showExplanation: e.target.checked})} />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm">Разбърквай въпросите</span>
            <input type="checkbox" checked={!!settings.shuffleQuestions} onChange={(e)=>setSettings({...settings, shuffleQuestions: e.target.checked})} />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm">Разбърквай отговорите</span>
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="text-sm">Лимит от време (минути, 0 = без)</span>
            <Input type="number" min={0} value={settings.timeLimitMin} onChange={(e)=>setSettings({...settings, timeLimitMin: Math.max(0, parseInt(e.target.value||"0"))})} className="w-24" />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-sm">Автоматично към следващия въпрос</span>
            <input type="checkbox" checked={!!settings.instantNext} onChange={(e)=>setSettings({...settings, instantNext: e.target.checked})} />
          </div>
          <div className="flex items-center justify_between gap-2">
            <span className="text-sm">Време до преминаване (сек.)</span>
            <Select disabled={!settings.instantNext} value={String(settings.instantDelaySec || 4)} onValueChange={(v)=>setSettings({...settings, instantDelaySec: parseInt(v,10)})}>
              ...
              <SelectContent>
                {delayOptions.map((s) => (
                  <SelectItem key={s} value={String(s)}>{s} сек.</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
...
export default function MathApp() {
  ...
  const [activeQuiz, setActiveQuiz] = useState(null); // { lesson, questions }
  ...
  const [settings, setSettings] = useLocalStorage(STORAGE_KEYS.settings, {
    ...
    shuffleOptions: true,
    timeLimitMin: 8, // minutes; 0 disables timer
    instantNext: false,
    instantDelaySec: 4,
  });
