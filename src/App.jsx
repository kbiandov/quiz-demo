...
function SquareButton({ label, onClick, icon = "🔹", subtitle }) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function HomeScreen({ profile, onGo }) {
  ...
  return (
    <div className="mx-auto max-w-4xl p-6">
      {/* Профилен ред */}
      <div className="mb-6 p-4 bg-white rounded-xl border border-slate-200 shadow-sm text-center">
...
function TheoryScreen() {
  return <div className="max-w-3xl mx-auto p-6 text-slate-700"><h2 className="text-xl font-semibold mb-3">Теория</h2><p>Тук може да показваме резюмета по уроци или да вградим външни материали. В момента е placeholder.</p></div>;
}

function ResultsScreen({ results, lessons, canRestart, onRestart }) {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      ...
      <div className="mt-6">
        <button type="button" className="btn" onClick={onRestart} disabled={!canRestart}>
          Започни отначало
        </button>
      </div>
    </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="card">
          <div className="card-content text-center">
            ...
            <div className="text-xs text-slate-500">Опити</div>
          </div>
        </div>
        <div className="card">
          <div className="card-content text-center">
            <div className="text-2xl font-bold">{totals.solved}</div>
            <div className="text-xs text-slate-500">Вярни</div>
          </div>
        </div>
        <div className="card">
          <div className="card-content text-center">
            <div className="text-2xl font-bold">{totals.totalQs}</div>
            <div className="text-xs text-slate-500">Въпроси</div>
          </div>
        </div>
        <div className="card">
          <div className="card-content text-center">
            <div className="text-2xl font-bold">{totals.accuracy}%</div>
            <div className="text-xs text-slate-500">Точност</div>
          </div>
        </div>
      </div>
    </div>
...
function TestsScreen({ profile, lessons, classes, questions, onStartQuiz }) {
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <h2 className="text-xl font-semibold mb-4">Избери тест</h2>
      <div className="space-y-6">
        {classList.map(cls => (
          <div key={cls.id}>
            <div className="text-lg font-semibold mb-3">{cls.name}</div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {(lessonsByClass[cls.id] || []).map(l => {
                const qs = listQuestionsForLesson(l.id);
                return (
                  <div key={normalizeId(l.id)} className="card hover:shadow-md transition">
                    <div className="card-content flex items-center justify-between">
                      <div>
...
function Quiz({ lesson, questions, onFinish, settings }) {
  ...
  function computeScore() {
    ...
    for (const q of qlist) {
      if ((answers[q.id] || '').toUpperCase() === (q.__correctKey || '').toUpperCase()) correct++;
    }
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center gap-4 text-sm text-slate-600">
            <div>Въпрос {index + 1} от {total}</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {opts.map(o => (
              <button
                type="button"
                key={o.key}
                className={`btn h-auto py-3 text-left ${chosen === o.key ? (isCorrect ? '' : 'btn-danger') : ''}`}
...
export default function MathApp() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50">
      {route === "home" ? (
        <>
          {/* Профилен ред само за началния екран */}
          ...
          <HomeScreen profile={profile} onGo={setRoute} />
        </>
      ) : (
        <>
