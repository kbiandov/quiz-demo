...
import { Settings, Home, Play, CheckCircle, BarChart3, BookOpen, ListChecks, User, LogOut } from "lucide-react";
...
// UI bits
function Progress({ value=0 }){ return <div className="h-2 bg-slate-200 rounded-full"><div className="h-full bg-blue-500 rounded-full" style={{width:`${value}%`}}/></div>; }
...
// Local Tabs (no external deps)
const TabsContext = React.createContext(null);
function Tabs({ defaultValue, children, className="" }){ const [value,setValue]=useState(defaultValue); const ctx=useMemo(()=>({value,setValue}),[value]); return <TabsContext.Provider value={ctx}><div className={className}>{children}</div></TabsContext.Provider>; }
function TabsList({ children, className="" }){ return <div className={className}>{children}</div>; }
function TabsTrigger({ value, children }){ const ctx=React.useContext(TabsContext); const active=ctx?.value===value; return <button type="button" onClick={()=>ctx.setValue(value)} className={`px-3 py-2 border rounded-md mr-2 ${active?'bg-blue-50 border-blue-300':'bg-white border-slate-200 hover:bg-slate-50'}`}>{children}</button>; }
function TabsContent({ value, children, className="" }){ const ctx=React.useContext(TabsContext); if(ctx?.value!==value) return null; return <div className={className}>{children}</div>; }

// Header
function Header({ title, profile, goHome, openSettings, resetProfile }){
  return (
    <div className="sticky top-0 bg-white/80 backdrop-blur border-b z-10">
      <div className="mx-auto max-w-4xl px-4 py-3 flex justify-between items-center">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-600">
          <User className="w-4 h-4"/><span>{profile?.name || "Гост"} · {profile?.classId? `Клас: ${profile.classId}`:"Без клас"}</span>
          <button type="button" onClick={resetProfile} className="border rounded-lg px-2 py-1"><LogOut className="inline w-4 h-4 -mt-1 mr-1"/>Смени профил</button>
...
function Onboarding({ classes, onDone }){
  ...
  return <div className="min-h-screen"><div className="mx-auto max-w-md px-4 py-10">
    ...
    <div className="space-y-4">
      <div><label className="block text-sm font-medium mb-1">Име / прякор</label><input className="w-full rounded-lg border border-slate-300 px-3 py-2" value={name} onChange={e=>setName(e.target.value)} placeholder="напр. Алекс"/></div>
...
function Home({ goto }){
  ...
  return <div className="mx-auto max-w-4xl p-6 grid grid-cols-2 md:grid-cols-4 gap-4">{tiles.map(t=><SquareTile key={t.key} icon={t.icon} label={t.label} onClick={()=>goto(t.key)}/>)}</div>;
}

function Theory(){ return <div className="max-w-3xl mx-auto p-6 text-slate-700"><h2 className="text-xl font-semibold mb-3">Теория</h2><p>Тук може да показваме резюмета по уроци или да вградим външни материали. В момента е placeholder.</p></div>; }
...
function Tests({ profile, lessons, questions, onStart }){
  ...
  const classList = useMemo(()=> {
    const ids = Array.from(new Set(lessons.map(l=> norm(l.class_id || l.classId || l.class || l.grade)).filter(Boolean)));
    return ids.map(id=>({ id, name: `${id} клас` }));
  }, [lessons]);
  ...
  return <div className="max-w-4xl mx-auto p-4">
    <Tabs defaultValue="by-lesson" className="w-full">
      <TabsList className="mb-3">
        <TabsTrigger value="by-lesson">По тема / уроци</TabsTrigger>
        <TabsTrigger value="by-class">По класове</TabsTrigger>
      </TabsList>

      <TabsContent value="by-lesson" className="pt-2">
        <h3 className="text-lg font-semibold mb-3">{`Уроци за ${currentClassId || "избрания клас"}`}</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          {currentLessons.map(l=>{
            ...
            return <div key={norm(l.id)} className="bg-white border rounded-2xl p-4 flex items-center justify-between">
              ...
              <button type="button" className="border px-3 py-2 rounded-lg disabled:opacity-50" onClick={()=>onStart(l,qs)} disabled={!qs.length}><Play className="inline w-4 h-4 -mt-1 mr-1"/>Старт</button>
            </div>;
          })}
          {!currentLessons.length && <div className="text-slate-600">Няма уроци за избрания клас.</div>}
        </div>
      </TabsContent>

      <TabsContent value="by-class" className="pt-2">
        <div className="space-y-4">
          {classList.map(cls=>{
            const list = lessonsByClass[cls.id] || [];
            return <div key={cls.id}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                {!list.length && <div className="text-slate-600">Няма уроци за този клас.</div>}
              </div>
            </div>;
          })}
        </div>
...
function Quiz({ lesson, questions, onFinish, settings }){
  ...
  const chosen = answers[cur.id]; const isCorrect = chosen && chosen.toUpperCase()===cur.__key; const showExp = settings?.showExplanation && chosen;
  function choose(k){
    setAnswers(a=>({...a,[cur.id]:k}));
    if (settings?.instantNext){
      if (autoRef.current){ clearTimeout(autoRef.current); autoRef.current=null; }
      const delay = Math.max(1, Number(settings.instantDelaySec||4))*1000;
      autoRef.current = setTimeout(()=>{ autoRef.current=null; if(i<total-1) setI(i+1); else setShow(true); }, delay);
    }
  }
  ...
  return <div className="fixed inset-0 bg-black/40 grid place-items-center z-50" onClick={onClose}>
    <div className="bg-white rounded-2xl p-4 w-[480px] max-w-[92vw]" onClick={e=>e.stopPropagation()}>
      ...
      <div className="space-y-3">
        ...
        <label className="flex justify-between items-center text_sm"><span>Разбърквай въпросите</span><input type="checkbox" checked={!!settings.shuffleQuestions} onChange={e=>setSettings({...settings, shuffleQuestions:e.target.checked})}/></label>
        <label className="flex justify-between items_center text-sm"><span>Разбърквай отговорите</span><input type="checkbox" checked={!!settings.shuffleOptions} onChange={e=>setSettings({...settings, shuffleOptions:e.target.checked})}/></label>
...
export default function App(){
  ...
  if (activeQuiz) return <div className="min-h-screen bg-slate-50"><Header title="Тест" profile={profile} goHome={()=>setActiveQuiz(null)} openSettings={()=>setSettingsOpen(true)} resetProfile={resetProfile} /><Quiz lesson={activeQuiz.lesson} questions={activeQuiz.questions} onFinish={finish} settings={settings}/>{settingsOpen && <SettingsModal settings={settings} setSettings={setSettings} onClose={()=>setSettingsOpen(false)} />}</div>;

  return <div className="min-h-screen"><Header title={titleOf(route)} profile={profile} goHome={()=>setRoute("home")} openSettings={()=>setSettingsOpen(true)} resetProfile={resetProfile}/>
