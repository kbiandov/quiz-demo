...
const BannerAd = () => (
  <div className="w-full border rounded-xl p-3 text-center text-xs text-gray-500 bg-gray-50">
    Реклама (банер)
  </div>
);
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-md space-y-4">
        ...
        <div className="h-40 border rounded-lg bg-gray-50 flex items-center justify-center text-gray-400">
          Ad slot
        </div>
        <button
          onClick={onClose}
...
function normQuestions(rows) {
  return rows
    .map((q) => {
      const opts = [q.option_1, q.option_2, q.option_3, q.option_4].filter(
        (v) => v && v.length
      );
    ...
    .filter(
      (x) => Number.isFinite(x.lesson_id) && x.q && x.options.length >= 2
    );
}
...
export default function QuizDemo({ initialFilters } = {}) {
  ...
  const [csvUrls, setCsvUrls] = useState(() => {
    try {
      ...
      return raw ? JSON.parse(raw) : DEFAULT_CSV_URLS;
    } catch {
      return DEFAULT_CSV_URLS;
    }
  });
  ...
  async function loadFromGoogleSheets() {
    ...
    try {
      ...
      if (required.some((u) => !u))
        throw new Error("Липсват CSV линкове. Попълни всички полета.");
  ...
  const lessonsForSubject = useMemo(
    () => db.lessons.filter((l) => l.subject_id === selectedSubjectId),
    [db.lessons, selectedSubjectId]
  );
  ...
  function abortTest() {
    const ok = window.confirm(
      "Сигурни ли сте, че искате да прекратите теста и да се върнете в началото?"
    );
    if (!ok) return;
  ...
  function restoreDefaults() {
    setDraftUrls(DEFAULT_CSV_URLS);
  }
  function saveSettings() {
    setCsvUrls(draftUrls);
    ...
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-gray-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl p-6 lg:p-8 space-y-6 relative">
        ...
        <button
          ...
          className="absolute right-4 top-4 rounded-full border p-2 hover:bg-gray-50"
        >
        ...
        {!started && db.questions.length > 0 && (
          <section className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const s =
                      db.subjects.find((x) => x.class_id === v)?.id ?? null;
                    setSelectedSubjectId(s);
                    const les =
                      db.lessons.find((x) => x.subject_id === s)?.id ?? null;
              ...
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const les =
                      db.lessons.find((x) => x.subject_id === v)?.id ?? null;
                    setSelectedLessonId(les);
                  }}
                >
              ...
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  value={questionCount}
                  onChange={(e) =>
                    setQuestionCount(Math.max(3, Number(e.target.value) || 6))
                  }
                  className="border rounded-xl px-3 py-2 focus:outline-none focus:ring w-full"
                />
              </div>
            ...
            <div className="grid grid-cols-2 gap-2 text-xs text-gray-600">
              <div>
                Класове: <span className="font-semibold">{db.classes.length}</span>
              </div>
              <div>
                Предмети:{" "}
                <span className="font-semibold">{db.subjects.length}</span>
              </div>
              <div>
                Уроци: <span className="font-semibold">{db.lessons.length}</span>
              </div>
              <div>
                Въпроси:{" "}
                <span className="font-semibold">{db.questions.length}</span>
              </div>
            </div>
            ...
            <div className="flex gap-3 items-center">
              <button
                onClick={handleStart}
                className="w-full md:w-auto inline-flex items-center justify-center gap-2 rounded-2xl px-6 py-3 bg-black text-white hover:opacity-90 active:scale-[0.99] shadow"
                disabled={db.questions.length === 0}
              >
                ▶ Стартирай теста
              </button>
              {db.questions.length === 0 && (
                <span className="text-xs text-red-600">
                  Няма въпроси — отвори ⚙️ Настройки и зареди от Sheets.
                </span>
              )}
            </div>
          </section>
          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-3">
                <div className="text-sm text-gray-600">
                  Точки: <span className="font-semibold">{score}</span>
                </div>
                <button
                  onClick={abortTest}
                ...
                {current?.options.map((opt, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={[
                        "text-left border rounded-xl px-4 py-3 transition shadow-sm",
        ...
        {started && answers.length === total && total > 0 && (
          <section className="space-y-5">
              ...
              {answers.map((a, i) => (
                <div key={i} className="border rounded-xl p-3 bg-gray-50">
                  ...
                  <div className="text-sm mb-1">
                    Твой отговор:{" "}
                    {a.selected >= 0
                      ? `${String.fromCharCode(65 + a.selected)}. ${a.options[a.selected]}`
                      : "(пропуснат)"}
                  </div>
                  <div className="text-sm">
                    Верен отговор: {String.fromCharCode(65 + a.correctIndex)}.{" "}
            ...
            <div className="flex items-center justify-center gap-3">
              <button
                onClick={restart}
                className="rounded-2xl px-5 py-2.5 border hover:bg-gray-50"
              >
                Начален екран
              </button>
              <button
      ...
      {showSettings && (
        <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl p-6 space-y-4">
            ...
            <div className="text-sm text-gray-600">
              Линковете се пазят локално в браузъра (localStorage).
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                value={draftUrls.classes}
                onChange={(e) =>
                  setDraftUrls({ ...draftUrls, classes: e.target.value })
                }
              />
              <label className="text-sm">Subjects CSV URL</label>
              <input
                className="border rounded-xl px-3 py-2 w-full"
                value={draftUrls.subjects}
                onChange={(e) =>
                  setDraftUrls({ ...draftUrls, subjects: e.target.value })
                }
              />
              <label className="text-sm">Lessons CSV URL</label>
              <input
                className="border rounded-xl px-3 py-2 w-full"
                value={draftUrls.lessons}
                onChange={(e) =>
                  setDraftUrls({ ...draftUrls, lessons: e.target.value })
                }
              />
              <label className="text-sm">Questions CSV URL</label>
              <input
                className="border rounded-xl px-3 py-2 w-full"
                value={draftUrls.questions}
                onChange={(e) =>
                  setDraftUrls({ ...draftUrls, questions: e.target.value })
                }
              />
            </div>

            {error && (
              <div className="text-xs text-red-600">Грешка: {error}</div>
            )}
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              >
                Възстанови стандартните линкове
              </button>
              <button
