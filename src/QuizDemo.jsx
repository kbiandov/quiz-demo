...
/**
 * QUIZ DEMO — BG UI + Settings + Google Sheets
 * - ⚙️ Панел „Настройки“ (горе вдясно) за 4 CSV URL-а (пази се в localStorage)
 * - initialFilters: { classId, lessonName, difficulty, questionCount, perQuestionSeconds }
 * - autoStart: ако е подаден, пропуска началната форма и стартира директно теста,
 *              щом има готов списък с въпроси за избраните филтри.
 */

const STORAGE_KEY = "quiz.csvUrls";

// Работещи CSV линкове (Публикуване в интернет → формат CSV)
...
// Фолбек данни (ако не успеем да заредим от Sheets)
const MOCK = {
...
function parseCsv(text) {
  // Парсер с поддръжка на кавички и запетаи вътре
  const rows = [];
  let cur = "";
  let inQuotes = false;
  let row = [];
...
const normClasses = (rows) =>
  rows.map((c) => ({ id: Number(c.id), name: c.name }))
      .filter((x) => Number.isFinite(x.id) && x.name);

const normSubjects = (rows) =>
  rows.map((s) => ({ id: Number(s.id), class_id: Number(s.class_id), name: s.name }))
      .filter((x) => Number.isFinite(x.id) && Number.isFinite(x.class_id) && x.name);

const normLessons = (rows) =>
  rows.map((l) => ({ id: Number(l.id), subject_id: Number(l.subject_id), name: l.name }))
...
export default function QuizDemo({ initialFilters, autoStart } = {}) {
  ...
  async function load() {
    ...
    try {
      ...
      // Приеми начални филтри (от „Избран тест“)
      if (initialFilters) {
        if (Number.isFinite(initialFilters.classId)) cid = initialFilters.classId;
        if (initialFilters.lessonName) {
          // Точно съвпадение по име; ако искаш — ще го направя частично/без регистър
          const byName = data.lessons.find((x) => x.name === initialFilters.lessonName);
          if (byName) {
            lid = byName.id;
  ...
  // Автостартиране: ако идваме от „Избран тест“, пропускаме формата
  useEffect(() => {
    if (!autoStart) return;       // няма автостарт
    if (loading) return;          // чакаме данните
    if (started) return;          // вече стартирал
  ...
  function abortTest() {
    if (!window.confirm("Сигурни ли сте, че искате да прекратите теста?")) return;
    setPendingAdAction("abort"); setShowInterstitial(true);
  }
  ...
  const showConfigScreen = !autoStart && !started;   // показваме формата само ако НЕ е autoStart
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-b from-white to-gray-50 flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl p-6 lg:p-8 space-y-6 relative">
        ...
        {/* Подготвяне (autoStart), без форма */}
        {autoStart && !started && (
          <div className="space-y-2">
            <h2 className="text-xl font-semibold">Подготвяме теста…</h2>
            <p className="text-sm text-gray-600">Зареждаме въпросите за избраната тема.</p>
            {error && <div className="text-xs text-red-600">{error}</div>}
          </div>
        )}

        {/* Конфигурация — показва се само ако НЕ е autoStart */}
        {showConfigScreen && (
          <section className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    const l = db.lessons.find((x) => x.subject_id === v)?.id ?? null;
                    setSelectedLessonId(l);
                  }}
                >
              ...
              <div className="flex flex-col gap-2">
                ...
                <input type="number" min={3} className="border rounded-xl px-3 py-2" value={questionCount}
                       onChange={(e) => setQuestionCount(Math.max(3, Number(e.target.value) || 6))}/>
              </div>

              <div className="flex flex-col gap-2">
                <label className="text-sm text-gray-700">Време за въпрос (секунди)</label>
                <input type="number" min={5} max={120} className="border rounded-xl px-3 py-2" value={perQuestionSeconds}
                       onChange={(e) => setPerQuestionSeconds(Math.max(5, Math.min(120, Number(e.target.value) || 20)))}/>
              </div>
            </div>

            <button onClick={handleStart} className="w-full md:w-auto rounded-2xl px-6 py-3 bg-black text-white"
                    disabled={filteredQuestions.length === 0}>▶ Стартирай теста</button>
            ...
            {["classes", "subjects", "lessons", "questions"].map((k) => (
              <div key={k} className="space-y-1">
                ...
                <input className="border rounded-xl px-3 py-2 w-full"
                       value={draftUrls[k]}
                       onChange={(e) => setDraftUrls({ ...draftUrls, [k]: e.target.value })}/>
              </div>
            ))}
